...
static char *english_share_name  = "HYPIM_ENGLISH_SHARED_NAME";
static char *engtrans_share_name = "HYPIM_ENGTRANS_SHARED_NAME";
//static char english_share_name[MAX_PATH];
//static char engtrans_share_name[MAX_PATH];
 ...
 *		失败：0
 */
int LoadEnglishData(const char *file_name)
{
	int file_length;
	...
	if (file_length <= 0)
		return 0;
    //strcpy_s(english_share_name, MAX_PATH, file_name);
	...
	if ((file_length = LoadFromFile(file_name, eng_wordlib, file_length)) == -1)
	{
		...
		printf("english.c : 英文词典文件打开失败。name=%s\n", file_name);
		return 0;
	}
...
int GetEnglishCandidates(const TCHAR *prefix, CANDIDATE *candidate_array, int array_length)
{
	...
	if (!eng_wordlib)
	{
		eng_wordlib = GetSharedMemory(english_share_name);
	...
	prefix_len = (int)utf16_strlen(prefix);
	if (!prefix_len)
		return 0;

	for (i = 0; i < (int)utf16_strlen(prefix); i++)
	{
		if (prefix[i] == '*')
		{
			has_star = 1;
	...
	Utf16ToAnsi((TCHAR*)prefix, prefix_char, sizeof(prefix_char));
	...
	for (i = m, count = 0; count < array_length && i < n; i++)
	{
		...
		if (has_star)
			...
			is_true = !strncasecmp(prefix_char, english_str, prefix_len);
 ...
 *		失败：0
 */
int LoadEnglishTransData(const char *file_name)
{
	int file_length;
	...
	if (file_length <= 0)
		return 0;
    //strcpy_s(engtrans_share_name, MAX_PATH, file_name);
	...
	if ((file_length = LoadFromFile(file_name, eng_translib, file_length)) == -1)
	{
		...
		printf("english.c : 英文翻译数据文件打开失败。name=%s\n", file_name);
		return 0;
	}
...
TCHAR* GetEnglishTranslation(const TCHAR *english_word)
{
	...
	if (!eng_translib)
	{
		eng_translib = GetSharedMemory(engtrans_share_name);
	...
	length = (int)utf16_strlen(english_word);
	if (length < 2)
		return 0;
	...
	for (i = m; i < n; i++)
	{
		if (length != utf16_strlen(eng_translib->data + eng_translib->EngIndex[i]))
			continue;
