 ...
 * 	keyval 	键码
 */
void FirstTimeResetContext(PIMCONTEXT *context, int keyval)
{
	...
	if(keyval == 0xFFE5)	//0xFFE5 Caps_Lock
		context->capital = 1;
...
void ClearCandidateInfo(PIMCONTEXT *context)
{
	/*
...
/*	清除写作信息
 */
void ClearComposeInfo(PIMCONTEXT *context)
{
...
TCHAR *GetReaminComposeString(PIMCONTEXT *context)
{
	...
	{
		if (utf16_strlen(string) > _SizeOf(string) - 8)
			break;
...
/*	切换英文候选模式
 */
int ToggleEnglishCandidate(PIMCONTEXT *context)
{
	if (!context || !pim_config)
        return -1;
