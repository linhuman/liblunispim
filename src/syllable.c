 ...
 *		0			不包含
 */
inline static int ProcessHUAndF(SYLLABLE syllable, SYLLABLE checked_syllable, int fuzzy_mode)
{
...
{
	int hi, low, mid, ret;
	int py_length = (int)utf16_strlen(pin_yin);
	...
	while(low <= hi)
	{
		...
		if (py_length != share_segment->syllable_map[mid].pin_yin_length)
			ret = utf16_strcmp(py, share_segment->syllable_map[mid].pin_yin);
		else
			ret = utf16_strncmp(py, share_segment->syllable_map[mid].pin_yin, py_length);
...
int GetSyllableFromU8py(const char *pin_yin, SYLLABLE *syllable, int *syllable_str_length, int fuzzy_mode)
{
    int hi, low, mid, ret;
    int py_length = (int)strlen(pin_yin);
    int has_tone = 0, has_separator = 0;	//是否有音调、音节分隔符
    TCHAR py[MAX_PINYIN_LENGTH + 1];		//备份拼音串
...
/*只比较声母和与韵母*/
int SyllableCmp(SYLLABLE syllable1, SYLLABLE syllable2)
{
    if(syllable1.con != syllable2.con)
        return syllable1.con - syllable2.con;
    if(syllable1.vow != syllable2.vow)
        return syllable1.vow - syllable2.vow;
    return 0;
}
...
int GetSPSyllable(const TCHAR *pin_yin, SYLLABLE *syllable, int *syllable_str_length, int fuzzy_mode)
{
	...
	{
		if (!utf16_strcmp(py, share_segment->syllable_map[i].sp_pin_yin) &&
	...
	syllable_length = (int)utf16_strlen(share_segment->syllable_map[i].sp_pin_yin);
	if (has_tone)
...
static int ProcessParse(const TCHAR *pin_yin, SYLLABLE *syllables, int array_length, int fuzzy_mode/*, int *correct_flag, int *separator_flag*/)
{
	...
	//py_length = (int)utf16_strlen(pin_yin);
		...
		for (i = 0; i < share_segment->syllable_map_items; i++)
		{
			...
			for (j = 0; j <= 3; j++)
			{
				...
				else
				{
					...
					_tcscat_s(str, length, tone_to_string[INT64_C(1) << j]);
				}
			}
...
int GetSyllableU8String(SYLLABLE syllable, char *str, int length, int tone_char)
{
    TCHAR *con_str, *vow_str;
    int count = 0;
    int i, j;
        ...
        (VOW_NULL == syllable.vow || VOW_END == syllable.vow || VOW_ANY == syllable.vow))
    {
        ...
        while(*con_str && count < length - 1)
            str[count++] = (char)*con_str++;

        while(*vow_str && count < length - 1)
            str[count++] = (char)*vow_str++;
    }
    //正常的拼音，查表获取，主要是为了实现拼音容错
    else
    {
        ...
        for (i = 0; i < share_segment->syllable_map_items; i++)
        {
            ...
            for (j = 0; j <= 3; j++)
            {
                ...
                if (tone_char)
                {
                    ...
                    {
                    case 0:
                        u16tou8cat_s(str, length, share_segment->syllable_map[i].pin_yin_tone1);
                        break;

                    case 1:
                        u16tou8cat_s(str, length, share_segment->syllable_map[i].pin_yin_tone2);
                        break;

                    case 2:
                        u16tou8cat_s(str, length, share_segment->syllable_map[i].pin_yin_tone3);
                        break;

                    case 3:
                        u16tou8cat_s(str, length, share_segment->syllable_map[i].pin_yin_tone4);
                        break;
                    }
                }
                else
                {
                    u16tou8cat_s(str, length, share_segment->syllable_map[i].pin_yin);
                    u16tou8cat_s(str, length, tone_to_string[INT64_C(1) << j]);
                }
            }
...
//		if (utf16_strlen(pinyin) + utf16_strlen(py) < pinyin_len - 1)
...
/*	装载双拼INI文件数据
 */
int LoadSPIniData(const char *ini_name)
{
	TCHAR *buffer;
	TCHAR line[0x400];
	...
	while(index < length)
	{
		...
		if (!utf16_strcmp(SECTION1, line))
			section = 1;
		else if (!utf16_strcmp(SECTION2, line))
			section = 2;
		else if (!utf16_strcmp(SECTION3, line))
			section = 3;

		if (!section)
			continue;

		sp_string1 = utf16_strtok(line, TEXT("="), &saveptr);
		sp_string2 = utf16_strtok(0, TEXT("="), &saveptr);
		...
		if (section == 1)
		{
			for (i = 0; i < CON_ANY; i++)
			{
				if (!utf16_strcmp(sp_string1, share_segment->con_sp_string_save[i]))
				{
		...
		if (section == 2)
		{
			for (i = 0; i < VOW_ANY; i++)
			{
				if (!utf16_strcmp(sp_string1, share_segment->vow_sp_string_save[i]))
				{
		...
		if (section == 3)
		{
			for (i = 0; i < VOW_ANY; i++)
			{
				if (!utf16_strcmp(sp_string1, share_segment->vow_sp_string_single_save[i]))
				{
	...
	for (i = 0; i < share_segment->syllable_map_items; i++)
	{
		...
		if (utf16_strlen(line) <= 2)
			_tcscpy_s(share_segment->syllable_map[i].sp_pin_yin, _SizeOf(share_segment->syllable_map[0].sp_pin_yin), line);
		else
			utf16_minfprintf(stdout, u"双拼编码存在错误, %s\n", share_segment->con_sp_string[con], share_segment->vow_sp_string[vow]);
	}
