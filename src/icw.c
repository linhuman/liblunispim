...
static char bigram_name[MAX_PATH];
...
/**	装载bigram数据
 */
int LoadBigramData(const char *name)
{
	if (bigram_data)			//已经装载
		return 1;
    /*
...
void NewEvaluateGroup(ICWITEMSET *icw_items, int group_no)
{
	...
	cd = IconvOpen("UTF-16", "GB18030");
	CodeConvert(cd, (char*)u"△", 2, triangle, 3);
	for (i = 0; i < count; i++)
	{
		char c0[0x20] = {0};
		...
		CodeConvert(cd, c0, items[i].length * 2, ci0, sizeof (ci0)); //为了改进性能，这里把原本的处理函数Utf16ToAnsi改成现在这个，避免重复调用iconv_open函数损耗性能

		next_group_no = group_no + items[i].length;
		if (next_group_no == icw_items->group_count)
		{

			items[i].value = GetBigramValue(bigram_data, ci0, triangle);
			items[i].next = 0;
			continue;
		}
		//在后续的组中找出最佳值
		...
		for (j = 0; j < next_count; j++)
		{
			...
			CodeConvert(cd, c1, next_items[j].length * 2, ci1, sizeof (ci1)); 	//为了改进性能，这里把原本的处理函数Utf16ToAnsi改成现在这个，避免重复调用iconv_open函数损耗性能
		...
		if (!group_no)				//开始位置，需要计算开始的结果
			items[i].value *= GetBigramValue(bigram_data, triangle, ci0);
