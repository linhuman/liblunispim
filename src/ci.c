...
void CheckAndUpgradeCiCache()
{
	...
	if (CI_CACHE_SIGNATURE == share_segment->ci_cache.signature)
	{
		...
		while(pos + item_length <= share_segment->ci_cache.length)		//需要判定最后一个词条不越界
		{
			...
			memcpy_s(&share_segment->ci_cache.cache[pos + WORDLIB_FEATURE_LENGTH], item_length, (char*)ci_uc, item_length);
 ...
 *		失败：0
 */
int LoadCiCacheData(const char *ci_cache_file_name)
{
	int length;
	...
	{
		share_segment->ci_cache.length = share_segment->ci_cache.max_used_id = 0;
		printf("ci.c : 读取词Cache文件出错，file:%s, length=%d\n", ci_cache_file_name, length);
		return 0;
	}
 ...
 *		失败：0
 */
int SaveCiCacheData(const char *ci_cache_file_name)
{
	...
	{
		printf("ci.c : 保存词Cache数据失败，file:%s\n", ci_cache_file_name);
		return 0;
	}
...
int GetCiInPageByWildLetter(PAGE *page, const TCHAR *letters, int letter_count,	CANDIDATE *candidate_array,	int candidate_length)
{
	...
	for (item = (WORDLIBITEM*) page->data; (char*)item < (char*) &page->data + page->data_length; item = GetNextCiItem(item))
	{
		...
		//判断音节是否符合
		if (!WildCompareSyllablesAndLetters(letters, (int)utf16_strlen(letters), item->syllable, item->syllable_length))
			continue;
...
int ProcessCiCandidate(SYLLABLE *syllable_array, int syllable_count, const TCHAR *letters, CANDIDATE *candidate_array, int candidate_length, int same_ci_syllable_length)
{
	...
	if (letters && (pim_config->ci_option & CI_USE_FIRST_LETTER) &&
			(int)utf16_strlen(letters) >= pim_config->first_letter_input_min_hz)
    {
		//在内存中的全部词库中查找候选
		wordlib_id = GetNextWordLibId(-1);
		while (wordlib_id != -1)
 ...
 *		第一个大于第二个 >0
 */
static int CompareCiCodeFreq(const void *cand1, const void *cand2)
{
	int ret;
	...
	//长度不同
	if (c1->word.item->ci_length != c2->word.item->ci_length)
		return c2->word.item->ci_length - c1->word.item->ci_length;

	//如果词的字不同则返回两个词的内码差
	ret = memcmp(c2->word.hz, c1->word.hz, sizeof(HZ) * c1->word.item->ci_length);
	if (ret)
		return ret;

	//如果两个词中有一个是被删除的，则排在前面
	if (c1->word.item->effective && !c2->word.item->effective)
		return 1;

	if (c2->word.item->effective && !c1->word.item->effective)
		return -1;

	//比较字频
	return c2->word.item->freq - c1->word.item->freq;
}
/*	词候选排重。
 *	参数：
 *		candidates			词候选数组
 ...
 */
static int CompareCi(const void *cand1, const void *cand2)
{
	...
	c1 = (const CANDIDATE*)cand1;
	c2 = (const CANDIDATE*)cand2;
	cache_pos1  = c1->word.cache_pos;
	cache_pos2  = c2->word.cache_pos;
