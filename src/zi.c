...
static char *hz_data_share_name = "HYPIM_HZ_DATA_SHARED_NAME";
...
static int CompareZi(const void *c1, const void *c2)
{
	int used_count1, used_count2;
	int freq1, freq2;
	const CANDIDATE *cand1, *cand2;
	cand1 = (const CANDIDATE*)c1;
	cand2 = (const CANDIDATE*)c2;
	if (cand1->hz.is_word)
	{
		freq1		= cand1->hz.word_item->freq;
		freq1		= AdjustWordFreqToHZFreq(freq1);
...
void GetZiBHPinyin(UC zi, TCHAR *buffer, int length)
{
	...
	if (!hz_data)
	{
		hz_data = GetSharedMemory(hz_data_share_name);
	...
	for (i = 0; i < hz_data->hz_count; i++)
	{
		...
		if (s_len + (int)utf16_strlen(buffer) + 2 >= length - 1)
			return;
...
int LoadHZData(const char *hz_data_name)
{
	int file_length;
	...
	if ((file_length = LoadFromFile(hz_data_name, hz_data, file_length)) == -1)
	{
        ...
        DEBUG_ECHO("汉字信息表文件打开失败。name=%s", hz_data_name);
		return 0;
	}
 ...
 *		失败：0
 */
int LoadTopZiData(const char *name)
{
	TCHAR *buffer;
	TCHAR line[0x400];
	TCHAR *saveptr = NULL;
	...
	while(share_segment->topzi_table_items < MAX_SYLLABLES && index < length)
	{
		...
		syllable_string = utf16_strtok(line, TEXT("="), &saveptr);
		zi_string = utf16_strtok(0, TEXT("="), &saveptr);
		...
		//检查字的长度
		zi_count = (int)utf16_strlen(zi_string);
 ...
 *	成功：1，失败：0
 */
int LoadZiCacheData(const char *zi_cache_name)
{
 ...
 *		成功：1，失败：0
 */
int SaveZiCacheData(const char *zi_cache_name)
{
...
/**	释放ZiCacheData
 */
int FreeZiCacheData(const char *zi_cache_name)
{
	...
	for (i = 0; i < hz_data->hz_count; i++)
	{
		...
		if (check_top_zcs_fuzzy)
		{
			...
			for (j = 0; j < topzi_count; j++)
				if (top_zi[j] == (HZ)(hz_data->hz_item[i].hz))	//if (top_zi[j] == LOWORD(hz_data->hz_item[i].hz))
					break;
		...
		//判断是否为置顶字
		for (j = 0; j < topzi_count; j++)
			if (top_zi[j] == (HZ)(hz_data->hz_item[i].hz))
				break;
...
int DeleteUnreadableZiCandidates(CANDIDATE *candidate_array, int count)
{
	...
	for (i = 0; i < count; i++)
	{
		...
		{
            if (!FontCanSupport( candidate_array[i].hz.item->hz ) )
				continue;
		}
		else
		{
 ...
 */
int CompareHzCodeFreq(const void *cand1, const void *cand2)
{
	const CANDIDATE *c1, *c2;
	assert(cand1 && cand2);
	c1 = (const CANDIDATE*)cand1;
	c2 = (const CANDIDATE*)cand2;
	//按内码排序
	if (c1->hz.item->hz != c2->hz.item->hz)
		return c1->hz.item->hz - c2->hz.item->hz;

	//按字频排序
	return c2->hz.item->freq - c1->hz.item->freq;
}
 ...
 */
int CompareSmallCiCode(const void *c1, const void *c2)
{
	int ret;
	const CANDIDATE *cand1, *cand2;
	assert(c1 && c2);
