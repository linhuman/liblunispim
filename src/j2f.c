...
static char *jf_share_name = "HYPIM_JF_SHARED_NAME";
//static char jf_share_name[MAX_PATH];
static int jf_count  = 0;
static int jf_loaded = 0;
...
int LoadJFResource()
{
    ...
    GetFileFullName(HZJF_FILE_NAME, file_name);

	file_length = GetFileLength(file_name);
	if (file_length <= 0)
		return 0;

	jf_info = (JFITEM*)AllocateSharedMemory(jf_share_name, file_length);
	if (!jf_info)
		return 0;
	...
	if ((file_length = LoadFromFile(file_name, jf_info, file_length)) == -1)
	{
		...
		printf("简繁对照表文件打开失败。name=%s", file_name);
		return 0;
	}
...
static UC ZiJ2F(UC zi)
{
	...
	if (!jf_info)
	{
        jf_info = (JFITEM*)GetSharedMemory(jf_share_name);
	...
	{
		JFITEM *p = (JFITEM*)((intptr_t)jf_info + i * sizeof(JFITEM));
...
{
	int i, len = (int)utf16_strlen(zi_string);
...
static int ProcessWordJ2F(TCHAR *zi_word)
{
    ...
    GetFileFullName(J2F_FILE_NAME, filename);
	...
	buffer[len] = '\0';
	ls = (int)utf16_strlen(zi_word);

	while(p = utf16_strstr(pb, zi_word))
	{
