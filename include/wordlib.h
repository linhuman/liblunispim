...
#define	J2F_FILE_NAME				"/zi/j2f.dat"		//简体繁体转换数据文件名称
#define	WORDLIB_NAME_PREFIX			"/wordlib/"		//词库文件的前缀
#define	WORDLIB_USER_SHORT_NAME		"user.uwl"					//用户词库文件的短名称
#define WORDLIB_SYS_FILE_NAME		"/wordlib/sys.uwl"	//系统词库文件名
#define WORDLIB_V3V5_FILE_NAME		"/wordlib/v3v5.uwl" //V3V5导入词库文件名
...
#define	WORDLIB_FILE_NAME_LENGTH		384							//词库文件名称的最大长度
...
//页定义，这个结构体大小为WORDLIB_PAGE_SIZE，相当词库一行的数据最大容量
...
//装载词库文件。
extern int LoadWordLibrary(const char *wordlib_name);
...
extern int LoadWordLibraryWithExtraLength(const char *lib_name, int extra_length, int check_exist);

//装载用户词库文件
extern int LoadUserWordLibrary(const char *wordlib_name);
...
//创建空的词库文件
extern int CreateEmptyWordLibFile(const char *wordlib_file_name, const TCHAR *name, const TCHAR *author, int can_be_edit);
...
//词库维护函数组
int UpdateWordLibrary(const TCHAR *wl_name, UPDATEITEM *update_items, int item_count, int flag);
int CheckAndUpdateWordLibrary(const TCHAR *wl_name);
...
int CreateWordLibrary(const char *wordlib_file_name, const char *text_file_name, int *ok_count, int *err_count, char *err_file_name, void *call_back);
int CheckAndUpgrandWordLibFromV5BKToV6(const TCHAR *wl_name);
int UpgradeWordLibFromV5ToV6(const char *wl_file_name);
...
//删词
int DeleteCiFromAllWordLib(TCHAR *ci_str, int ci_length, TCHAR *py_str, int py_length);
