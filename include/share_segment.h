...
static char *segment_share_name = "HYPIM_SEGMENT_SHARED_NAME";
typedef struct tagSHARE_SEGMENT
{
	...
	char wordlib_name[MAX_WORDLIBS * 2][WORDLIB_FILE_NAME_LENGTH];	//词库的名称
	int	wordlib_length[MAX_WORDLIBS * 2];							//词库长度数组
	int wordlib_deleted[MAX_WORDLIBS * 2];							//词库是否已经被删除
    const char wordlib_shared_name[MAX_WORDLIBS * 2][0x20];				//共享内存的标识
	...
	//unsigned long long main_window_list[MAX_STATUS_LIST_SIZE];		//写作栏句柄集合
	//unsigned long long status_window_list[MAX_STATUS_LIST_SIZE];		//状态栏句柄集合
	//unsigned long long hint_window_list[MAX_STATUS_LIST_SIZE];		//提示栏句柄集合
