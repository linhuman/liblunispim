...
extern int BackspaceChar(PIMCONTEXT *context);
extern void ProcessContext(PIMCONTEXT *context);
extern void ProcessCandidateStuff(PIMCONTEXT *context);
extern int PrevCandidatePage(PIMCONTEXT *context);
extern int NextCandidatePage(PIMCONTEXT *context);
...
extern void MoveCursorLeftBySyllable(PIMCONTEXT *context);
...
extern int GetSelectedItemSyllable(SELECT_ITEM *item, SYLLABLE *syllables, int length);
